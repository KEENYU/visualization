...
int HeatMapper::increase(int x, int y, int delta)
{
    ...
    data_[index] += delta;
    qDebug()<<"cnt = "<<data_[index]<<"   max = "<<max_;  //一直是1?
	return data_[index];
}
...
{
    if (x <= 0 || y <= 0 || x > width_ || y > height_)  //出界判断
		return;

    int count = increase(x, y);  //增加并返回指定坐标点的命中次数

    //根据点击数产生余辉效果，数量多的明显，否则变淡
    if (max_ < count) {  //默认最大命名数max为1
        max_ = count;
        redraw();
        return;
    }

    drawAlpha(x, y, count);  //绘制透明径向渐变
}
...
{
    QColor color(0, 0, 0, 0);  //黑色，0代表透明无色
    alphaCanvas_->fill(color);  // 用于存储渐变透明数据的图像副本
    mainCanvas_->fill(color);  // 用于显示输出的图像

	int size = data_.size();
	for (int i = 0; i < size; ++i) {
        if (data_[i] == 0)
			continue;
        drawAlpha(i % width_ + 1, i / width_ + 1, data_[i], false);  //绘制透明径向渐变
	}
    colorize();  //着色
}
...
void HeatMapper::drawAlpha(int x, int y, int count, bool colorize_now)
{
    ...
    painter.setBrush(gradient);  //设置画笔
    painter.drawEllipse(QPoint(x, y), radius_, radius_);  //画圆
...
void HeatMapper::colorize(int left, int top, int right, int bottom)
{
	...
	for (int i = left; i < right; ++i) {
		for (int j = top; j < bottom; ++j) {
			...
			if (!alpha)
				continue;
            finalAlpha = (alpha < opacity_ ? alpha : opacity_);  //最终不透明度取小
